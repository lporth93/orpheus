...
void alloc_Gammans_discrete_ggg(
    double *isinner, double *weight, double *pos1, double *pos2, double *e1, double *e2, int *zbins, int nbinsz, int ngal, 
    ...
    int nthreads, int verbose, double *bin_centers, double complex *Gammans, double complex *Gammans_norm){
...
void alloc_Gammans_doubletree_ggg(
    ...
    for(int elthread=0;elthread<nthreads;elthread++){
        ...
        for (int _elregion=0; _elregion<2*nfilledregions; _elregion++){
            ...
            double innergal, pos1_gal1, pos2_gal1, pos1_gal2, pos2_gal2, w_gal1, w_gal2, wsq_gal2, e1_gal1, e2_gal1, e1_gal2, e2_gal2;
            double rel1, rel2, dist;
                    ...
                    for (int thisn=0; thisn<nmax+1; thisn++){
                        ...
                        for (int zrcombis1=0; zrcombis1<nallowedcounts; zrcombis1++){
                            ...
                            if (dccorr==1){
                                ...
                                gammashift = (gammashift1 + elb1_full);
                                //phirotm = wshape_gal1*nextG2ns[zrshift];
                                tmpGamma0s[gammashift] += wshape_gal1*nextG2ns[0*nbinszr_reso + zrshift];
                                tmpGamma1s[gammashift] += conj(wshape_gal1)*nextG2ns[1*nbinszr_reso + zrshift];
                            ...
                            for (int zrcombis2=0; zrcombis2<nallowedcounts; zrcombis2++){
                                ...
                                gammashift = (gammashift1 + elb2_full);
                                Gmnm3 = nextGns[ind_mnm3 + zrshift];
                                tmpGamma0s[gammashift] += h0*Gmnm3;
                                tmpGamma1s[gammashift] += h1*nextGns[ind_mnm1 + zrshift];
                                tmpGamma2s[gammashift] += h2*Gmnm3;
                                tmpGamma3s[gammashift] += h3*conj(nextGns[ind_nm1 + zrshift]);
                                    ...
                                    for (int elgal=0; elgal<ngal_in_pix[zbin1*nresos+thisreso2]; elgal++){
                                        for (int elb1=rbinmin1; elb1<rbinmax1; elb1++){
                                            ...
                                            for (int elb2=rbinmin2; elb2<rbinmax2; elb2++){
                                                ...
                                                //    cumresoshift_z[zbin1*(nresos+1) + thisreso2] + elgal;
                                                //ind_Gncacheshift = (nmax+3)*nshift + ind_Nncacheshift;
                                                gammashift = (gammashift1 + elb2);
                                                tmpGamma0s[gammashift] += h0*Gncache[_imnm3];
                                                tmpGamma1s[gammashift] += h1*Gncache[_imnm1];
                                                tmpGamma2s[gammashift] += h2*Gncache[_imnm3];
                                                ...
                                                //gammashift = (gammashift1 + elb2);
                                                //tmpGamma0s[gammashift] += h0*Gncache[thisthetshift + ind_Gncacheshift_mnm3];
                                                //tmpGamma1s[gammashift] += h1*Gncache[thisthetshift + ind_Gncacheshift_mnm1];
                                                //tmpGamma2s[gammashift] += h2*Gncache[thisthetshift + ind_Gncacheshift_mnm3];
                                                //tmpGamma3s[gammashift] += h3*conj(Gncache[thisthetshift + ind_Gncacheshift_nm1]);
                                    ...
                                    for (int elgal=0; elgal<ngal_in_pix[zbin1*nresos+thisreso1]; elgal++){
                                        for (int elb1=rbinmin1; elb1<rbinmax1; elb1++){
                                            ...
                                            for (int elb2=rbinmin2; elb2<rbinmax2; elb2++){
                                                ...
                                                //    cumresoshift_z[zbin1*(nresos+1) + thisreso1] + elgal;
                                                //ind_Gncacheshift = (nmax+3)*nshift + ind_Nncacheshift;
                                                tmpGamma0s[gammashift] += h0*wGncache[_imnm3];
                                                tmpGamma1s[gammashift] += h1*cwGncache[_imnm1];
                                                tmpGamma2s[gammashift] += h2*wGncache[_imnm3];
                                                ...
                                                _in += _thetashift_z;
                                                //thisthetshift = elb2*thetashifts_z[zbin1];
                                                //tmpGamma0s[gammashift] += mycmul(h0, wGncache[thisthetshift + ind_Gncacheshift_mnm3]);
                                                //tmpGamma1s[gammashift] += mycmul(h1,cwGncache[thisthetshift + ind_Gncacheshift_mnm1]);
                                                //tmpGamma2s[gammashift] += mycmul(h2,wGncache[thisthetshift + ind_Gncacheshift_mnm3]);
                                                //tmpGamma3s[gammashift] += mycmul_zzc(h3,cwGncache[thisthetshift + ind_Gncacheshift_nm1]);
            ...
            for (int zcombi=0; zcombi<nbinsz*nbinsz*nbinsz; zcombi++){
                for (int elb1=0; elb1<nbinsr; elb1++){
                    for (int elb2=0; elb2<nbinsr; elb2++){
                        iGamma = thisn*_gamma_nshift + zcombi*_gamma_zshift + elb1*nbinsr + elb2;
                        itmpGamma = iGamma + thisthread*_gamma_compshift;
                        Gammans[0*_gamma_compshift+iGamma] += tmpGamma0s[itmpGamma];
                        Gammans[1*_gamma_compshift+iGamma] += tmpGamma1s[itmpGamma];
                        Gammans[2*_gamma_compshift+iGamma] += tmpGamma2s[itmpGamma];
                        Gammans[3*_gamma_compshift+iGamma] += tmpGamma3s[itmpGamma];
                        Gammans_norm[iGamma] += tmpGammans_norm[itmpGamma];
                    }
                }
            }
...
void alloc_Gammans_discrete_GNN(
    double *isinner_source, double *w_source, double *pos1_source, double *pos2_source, double *e1_source, double *e2_source, int *zbin_source, int nbinsz_source, int ngal_source,
    ...
    double pix1_start, double pix1_d, int pix1_n, double pix2_start, double pix2_d, int pix2_n,
    int nthreads, int verbose, double *bin_centers, double complex *Upsilon_n, double complex *Norm_n){
...
void alloc_Gammans_doubletree_GNN(
    int nresos, int nresos_grid, double *dpix1_resos,  double *dpix2_resos, double *reso_redges, 
    int resoshift_leafs, int minresoind_leaf, int maxresoind_leaf,
    double *isinner_source_resos, double *w_source_resos, double *pos1_source_resos, double *pos2_source_resos, 
    double *e1_source_resos, double *e2_source_resos, int *zbin_source_resos, int *ngal_source_resos, int nbinsz_source, 
    double *isinner_lens_resos, double *w_lens_resos, double *pos1_lens_resos, double *pos2_lens_resos, 
...
void alloc_Gammans_discrete_NGG(
    ...
    double *isinner_lens, double *w_lens, double *pos1_lens, double *pos2_lens, int *zbin_lens, int nbinsz_lens, int ngal_lens, 
    int nmax, double rmin, double rmax, int nbinsr, int dccorr,
    int *index_matcher_source, int *pixs_galind_bounds_source, int *pix_gals_source, 
    int *index_matcher_lens, int *pixs_galind_bounds_lens, int *pix_gals_lens, int nregions, 
    double pix1_start, double pix1_d, int pix1_n, double pix2_start, double pix2_d, int pix2_n,
